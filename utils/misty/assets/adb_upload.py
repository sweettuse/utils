...
def upload_audio_to_misty(d, ip='', port=5555):
    """
    ...
    NOTE: you need to install adb (android debug bridge) on your system
    NOTE: reboot misty after the upload is complete so she can recognize the new files
    """

    os.chdir(d)
    commands = [f'adb connect {ip or os.environ["MISTY_IP"]}:{port}']
